...
[N] = size(parent_selected,1); %% sample number
xl1=xl'; %% Minimum boundary vector
xu1=xu'; %% Maximum boundary vector
rc=randperm(N); %% Randomly generated cross parent band number
for i=1:(N/2)
    parent1=parent_selected((rc(2*i-1)),:); %% Parent1 and parent2 are row vectors.
    ...
    if (isequal(parent1,parent2))==1 & rand(1)>0.5
        ...
        betaq=real((u<=(1./alpha)).*(u.*alpha).^(1/(etac+1))+(u>(1./alpha)).*(1./(2 - u.*alpha)).^(1/(etac+1))); %% Calculate polynomial probability distribution parameters
        child1=0.5*(((1 + betaq).*parent1) + (1 - betaq).*parent2); %% child1 and child2 are the offspring obtained by crossing.
