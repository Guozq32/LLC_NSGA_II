...
for i = 1: pop_size
 ...
 if pool(parent(1),rank)~=pool(parent(2),rank)              % For parents with different rank 
...
parent_selected(i,:)=mincandidate;                          % Minimum rank individual is selected finally
    ...
    elseif pool(parent(1),distance)< pool(parent(2),distance)  %%Randomly selected the most parent sample with high level, that is, low layer technology.
        maxcandidate=pool(parent(2),:);
    else
        temp=randperm(2);   %% If the two sample levels and crowding degrees are the same, one of them is randomly selected.
