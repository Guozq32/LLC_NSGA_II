...
global Po pop_size Vo Vin_min Vin_max rou miu ds diso xl xu  Ae Aw Ve AeAw h hw Icm Sm DL tdead Coss eta tf alpha beta kcof Ron
...
%% According to the steady-state solution results, the specifications and winding of the magnetic core are selected and determined.
...
for i=1:pop_size%%The magnetic core is selected by AP method, and the parameters of the magnetic core variable are determined.
    
    if Flag_error(i)==0  

        %----------Magnetic selection core of transformer-----------%
        ...
        %----------Select the wire diameter for the transformer-----------%
        ...
        %----------Determine the number of turns of the transformer-----------%
        Nl1(i)=floor(h_T(i)/DL1(i));  %The number of turns per layer
        ...
        %-------------Select the magnetic core for the inductor---------------%
...
%         Lr(i)=N3(i)*Bm_L(i)*Ae_L(i)/Ipeak_max(i);
%         %这里不能不修正吗，实际上用气隙来修正,Lr一变谐振槽的参数都要变
...
for i=1:pop_size
    ...
    if Km_T(i)>0.17 || Km_L(i)>0.17
        ...
        Rac1(i)=Rdc1(i)*Fac1(i);  
        Rac2(i)=Rdc2(i)*Fac2(i);  
        Rac3(i)=Rdc3(i)*Fac3(i);  
