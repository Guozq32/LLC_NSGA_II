function [x,Para,Para_cell,ff,err] = Cal_Loss(x) %%x=[n,Lr,fs,Lm, Bm_T,Bm_L]
...
%% 每次执行cal函数，初始化全局变量，不然之前的不会清除(可以没有， 二、或者把参数传递的这些参数写成table表的形式， 三、或者不用函数，直接把代码放进来)
...
J=500e4; %单位A/m^2； 电流密度500A/cm^2，也就是每平方mm通过5A电流

Kw_Tr=0.18;%%变压器窗口利用系数
Kw_Lr=0.18;

%% 通过for循环将每个参量确定限制在取值区间内
...
Irms_s_max=zeros(pop_size,1);       %%% 二次侧的电流有效值，依此选择二次侧变压器
Nl1=zeros(pop_size,1);%%应该是每层可绕下的线圈数
...
for i=1:pop_size%%AP法选磁芯，并确定磁芯变量的参数
    
    if Flag_error(i)==0  %首先确保参数满足要求，是可行解

        %----------变压器选磁芯-----------%
          AeAw_Tr(i)=(1+eta)*Po/(4*eta*Bm_T(i)*J*Kw_Tr*fs_min(i)); %与频率相关
        ...
        %----------进行变压器匝数选取-----------%
        Nl1(i)=floor(h_T(i)/DL1(i));  %每层绕的匝数
        Nl2(i)=floor(h_T(i)/DL2(i));
        N2(i)=Vo/(4*Bm_T(i)*Ae_T(i)*fs_min(i)); %与频率相关
        N1(i)=N2(i)/n(i);
        N2_floor(i)=floor(N2(i));%%向下取整
        N1_floor(i)=floor(N1(i));
        N2_ceil(i)=ceil(N2(i));%%向上取整
        N1_ceil(i)=ceil(N1(i));
...
for i=1:pop_size
    %----------填充系数交流电阻等相关参数计算-----------%
    Km_T(i)=(pi*(ds/2)^2*Sm1(i)*N1(i)+pi*(ds/2)^2*Sm2(i)*N2(i))/Aw_T(i); %%验证填充系数
    Km_L(i)=(pi*(ds/2)^2*Sm3(i)*N3(i))/Aw_L(i); %%验证填充系数
    if Km_T(i)>0.205 || Km_L(i)>0.205
        error_3(i)=100;
        Flag_error_3(i)=1;
    end
            lp(i)=sqrt(Ae_T(i)/pi)+(m1(i)*DL1(i)+diso*(m1(i)-1))*0.5; %%原边绕组平均半径
            lw(i)=sqrt(Ae_T(i)/pi)+m1(i)*DL1(i)+diso*m1(i); %%原副边绝缘层平均半径
            ls(i)=lw(i)+(m2(i)*DL2(i)+diso.*(m2(i)-1))*0.5; %%副边绕组平均半径
            lr(i)=sqrt(Ae_L(i)/pi)+(m3(i)*DL3(i)+diso*(m3(i)-1))*0.5; %%电感绕组平均半径
            Rdc1(i)=rou*2*pi*lp(i)*N1(i)/(pi*(ds/2)^2*Sm1(i));  %%通过计算平均线长度计算原边绕组直流电阻
            Rdc2(i)=rou*2*pi*ls(i)*N2(i)/(pi*(ds/2)^2*Sm2(i));  %%通过计算平均线长度计算副边绕组直流电阻
            Rdc3(i)=rou*2*pi*lr(i)*N3(i)/(pi*(ds/2)^2*Sm3(i));  %%通过计算平均线长度计算电感绕组直流电阻
    
%             Ddelta(i)=2^0.5/sqrt(2*pi*fr(i)*miu/rou); %集肤深度 ，这里改不改频率影响很小
%     
%             kxi1(i)=0.5*ds/Ddelta(i)*sqrt(pi*Nl1(i)*sqrt(Sm1(i)*pi)*ds*0.5/h_T(i));  %%修正系数
%             kxi2(i)=0.5*ds/Ddelta(i)*sqrt(pi*Nl2(i)*sqrt(Sm2(i)*pi)*ds*0.5/h_T(i));  %%修正系数
%             kxi3(i)=0.5*ds/Ddelta(i)*sqrt(pi*Nl3(i)*sqrt(Sm3(i)*pi)*ds*0.5/h_L(i));  %%修正系数
%     
...
%    
%         Rac1(i)=Rdc1(i)*Fac1(i)*factor;  %%原边绕组交流电阻
%         Rac2(i)=Rdc2(i)*Fac2(i)*factor;  %%副边绕组交流电阻
%         Rac3(i)=Rdc3(i)*Fac3(i)*factor;  %%电感绕组交流电阻
...
Ploss_avg=real(Ploss_avg);
P_variance=zeros(pop_size,1);
P_var_st=zeros(pop_size,1);
for i=1:pop_size
    for j=1:11
        P_variance_sum(i,j)=(P_given(i,j)-Ploss_avg(i))^2;
    end
    P_variance(i)=sum(P_variance_sum(i,:))/11;
    P_var_st(i)=sqrt(P_variance(i));
end
alpha_variance=0.35;
f_loss=Ploss_avg+alpha_variance*P_variance;
%%%%%%%%%%%%%%----回传数据-----%%%%%%%%%%%%%%%
ff=[f_loss Ve_T+Ve_L];
...
Para=[N1 N2 N3 Nl1 Nl2 Nl3 m1 m2 m3 Rac1 ...
    ...
    P_Lr_Fe_avg  P_recovery_Diode_Matrix_avg Ipeak_max ILs_rms_max  fs_max fs_min  error];%删掉了Range_fs
